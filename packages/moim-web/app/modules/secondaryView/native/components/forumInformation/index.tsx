...
import {
  useHandlers,
  useProps,
} from "app/modules/channelInformation/hooks/useForum";
...
export default function ForumInformation({ channelId }: { channelId: string }) {
  const { forum, isLoading, members, handleGetMembers } = useHandlers(
    useProps(channelId),
  );
  const {
