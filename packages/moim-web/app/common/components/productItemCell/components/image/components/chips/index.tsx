...
const CoinChipComponent: React.FC<{
  coinId: Moim.Id;
  visibleCoinName: boolean;
}> = ({ coinId, visibleCoinName }) => {
  ...
  return (
    <CoinChip badgeColor={coin.preview?.hexCode} scale={coin.preview?.fogType}>
      ...
      {visibleCoinName ? <ShavedText line={1} value={coin.name} /> : null}
    </CoinChip>
  );
};
interface IProps {
...
const HeaderChips = ({ productId, productSets }: IProps) => {
  ...
  const chips: ThumbnailChipType[] = React.useMemo(() => {
    ...
    return [
      ...availableProductSetChips,
      ...availableAdditionalFeeInfoChips,
    ].slice(0, 3);
  }, [additionalFeeInfos, productSets]);

  const visibleCoinChipCount = React.useMemo(
    () => chips.filter(chip => chip.type === "coin")?.length,
    [chips],
  );
  if (!chips?.length) {
    return null;
      ...
      {chips.map(({ type, value }) => {
        ...
        return (
          <CoinChipComponent
            ...
            visibleCoinName={visibleCoinChipCount <= 1}
          />
        );
      })}
    </TimeSaleChipContainer>
