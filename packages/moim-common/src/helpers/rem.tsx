import { PIXEL_TO_REM_RATIO, REM_STANDARD_WIDTH } from "../constants/default";
...
export function standardPixelConvert(pixel: number) {
  const windowWidth = window.innerWidth ?? REM_STANDARD_WIDTH;
...
export function rem2px(value: number) {
  ...
  const standardPixel = parseFloat(windowFontSize ?? "10px");
